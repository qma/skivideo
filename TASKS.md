...
## In Progress
- [handoff] Improve Live-Timing confirmation UI. In `public/app.js`, `renderLiveTimingSelection()` currently lists candidate race checkboxes with name/resort/date/confidence only. Add direct links for each candidate so an operator can inspect Live-Timing before confirming. Use the existing candidate fields stored in `folder.eventMatch.liveTimingCandidates`: `sourceUrl`, `raceId`, `gender`, `type`, `resort`, `date`, `confidence`, and `name`. The UI should open links in a new tab with `target="_blank" rel="noreferrer"` and should not trigger checkbox toggles accidentally when the link is clicked. Also consider exposing any available race report links if they are later added to serialized candidates.
- [handoff] Support confirming an event with no Live-Timing match. Some folders are training/practice events and should be allowed to proceed without Live-Timing. Add an explicit admin action in the same confirmation panel, for example `Confirm No Live-Timing`, that transitions `eventMatch.liveTimingSelection.status` to something like `admin_confirmed_none` or `no_live_timing`, clears `liveTimingMatches`, `liveTimingCandidates`, roster, and race assets except generic search/archive assets, and then resumes relabeling using filename/transcript-only evidence. Backend should expose this as a codified endpoint rather than client-side mutation, likely near `/api/confirm-live-timing` and `confirmLiveTimingSelection()` in `src/pipeline/eventDependencies.mjs`.
- [handoff] State-transition expectations for the no-match flow: `needs_admin_selection` -> `admin_confirmed_none` -> relabel/prepared/processed. `prepare-folder` and `process-folder` should no longer block once the admin explicitly confirms no Live-Timing; they should continue with empty roster context. Make sure `hasLiveTimingCorrelation()` treats this terminal no-match confirmation as resolved, while `needsLiveTimingSelection()` remains false.
- [handoff] Validate the Live-Timing confirmation continuation on currently pending local events: `GS Dec 31st, 2025`, `GS Race Jan 11. Northstar Day 3`, `SL Champs, April 5`, `SL Champs, Camera 2. April 5`, and `Super G Race 1 China Peak Feb 6`. After confirming a race pair or no-match, verify labels are recalculated immediately and event status no longer shows `Needs Live`.
