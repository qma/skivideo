...
- [gemini] Improved Live-Timing confirmation UI in `public/app.js`: added direct links to candidate race reports for easier operator inspection.
...
- [x] [BUG][high] Golden-label videos are demoted from `indexed` back to `needs_review` by Relabel / Process / Re-Process. `reviewVideo` (manual-label in `src/server.mjs`) sets `goldenLabel` and `processing.status = "indexed"`, but `relabelFolder`, `processVideo`, and the unified-session labeling phase in `src/pipeline/processFolder.mjs` recompute `processing.status` purely from `athleteLabels` prediction confidence and ignore `goldenLabel`. Reproduced live: set a golden label on `P1020609.MP4` (GS Camp April 24), relabeled the folder, and the status flipped to `needs_review` even though the golden label was retained — corrupting the indexed/review counts and review workflow. Fixed (subagent) by adding a shared `resolveLabeledStatus(video, bestConfidence)` helper that returns `indexed` whenever `goldenLabel` is present, used at all three sites. Verified live: the golden video stays `indexed` after relabel even with weak predictions.
- [x] [BUG][medium] Unified multi-turn session labeling is skipped for the entire event when any single video fails ingestion. `processFolder` in `src/pipeline/processFolder.mjs` guarded phase 2 with `if (useUnifiedSession && !failed)`, so one failed download/transcription left every non-failed video unlabeled and stuck in `needs_review` (they were ingested with `skipLabeling: true`). Fixed (subagent) by dropping the `&& !failed` guard; the phase-2 loop already skips per-video failures via `if (video.processing?.status === "failed") continue;`.
